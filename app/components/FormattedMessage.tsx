...
export default function FormattedMessage({ content }: FormattedMessageProps) {
  ...
  const processMarkdownAndLinks = (text: string) => {
    ...
    processedText = processedText.replace(
      /^### (.*$)/gm,
      '<h3 class="font-semibold text-lg my-2">$1</h3>',
    );
    processedText = processedText.replace(
      /^## (.*$)/gm,
      '<h2 class="font-semibold text-xl my-2">$1</h2>',
    );
    processedText = processedText.replace(
      /^# (.*$)/gm,
      '<h1 class="font-bold text-2xl my-3">$1</h1>',
    );
    ...
    processedText = processedText.replace(
      /`(.*?)`/g,
      '<code class="bg-gray-100 px-1 rounded font-mono text-sm">$1</code>',
    );

    // Handle blockquotes
    processedText = processedText.replace(
      /^> (.*$)/gm,
      '<blockquote class="border-l-4 border-gray-300 pl-3 my-2 italic text-gray-600">$1</blockquote>',
    );
        ...
        if (index % 2 === 0) {
          return part.replace(
            ...
            (match, prefix, url) => {
              ...
              return (
                prefix +
                `<a href="${cleanUrl}" target="_blank" class="text-blue-500 hover:underline">${cleanUrl}</a>`
              );
            },
          );
        }
