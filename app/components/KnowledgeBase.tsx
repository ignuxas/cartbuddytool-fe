  ...
  const handleUploadFile = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast({
          title: data.error || t("project.uploadFailed"),
          color: "danger",
        });
      }
    } catch (_error) {
  ...
  const handleUpdateFile = async (fileId: string, file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast({
          title: data.error || t("project.updateFailed"),
          color: "danger",
        });
      }
    } catch (_error) {
  ...
  const handleDownload = async (fileId: string) => {
    ...
    try {
      const res = await fetch(
        `${config.serverUrl}/api/knowledge/download/?domain=${domain}&file_id=${fileId}`,
        {
          headers: { Authorization: `Bearer ${authKey}` },
        },
      );
      ...
      if (res.ok && data.download_url) {
        ...
        addToast({
          title: t("project.getDownloadLinkFailed"),
          color: "danger",
        });
      }
    } catch (_error) {
                ...
                <TableRow key={file.id}>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-default-400 bg-default-100 px-1 rounded">
                        {file.file_type}
                      </span>
                    </div>
                  </TableCell>
