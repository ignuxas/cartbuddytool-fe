...
export default function MetricsPage() {
  ...
  const {
    ...
    isLoading: loading,
    error: metricsError,
  } = useMetricsDashboard(domain, authKey);
  ...
  const [paginatedInteractions, setPaginatedInteractions] = useState<
    MetricsData["recent_metrics"]
  >([]);
  const [paginatedErrors, setPaginatedErrors] = useState<
    MetricsData["recent_errors"]
  >([]);
  ...
  const toggleChartFilter = (chart: "daily" | "hourly", key: string) => {
    setChartFilters((prev) => ({
      ...
      [chart]: {
        ...prev[chart],
        [key as keyof (typeof prev)[typeof chart]]:
          !prev[chart][key as keyof (typeof prev)[typeof chart]],
      },
    }));
  };
  ...
  useEffect(() => {
    ...
    if (interactionsPage === 1) {
      setPaginatedInteractions(
        normalizedData.recent_metrics.slice(0, ITEMS_PER_PAGE),
      );
  ...
  return (
    <div className="flex flex-col gap-6 py-6 transition-all duration-300 ease-in-out">
      ...
      {/* Charts Row 1 */}
      <div
        className={`grid grid-cols-1 ${fullscreenChart ? "" : "lg:grid-cols-2"} gap-6 mb-6`}
      >
        ...
        >
          <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-6 py-4 gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  size="sm"
                  variant={
                    chartFilters.daily.interactions ? "solid" : "bordered"
                  }
            ...
            <Button
              ...
              variant="light"
              onPress={() =>
                setFullscreenChart(fullscreenChart === "daily" ? null : "daily")
              }
            >
              {fullscreenChart === "daily" ? (
                <svg
          ...
          <CardBody
            className={
              fullscreenChart === "daily" ? "h-[calc(100vh-100px)]" : ""
            }
          >
            {metrics.daily_stats.length > 0 ? (
              <ResponsiveContainer
                height={fullscreenChart === "daily" ? "100%" : 300}
                width="100%"
              >
        ...
        >
          <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-6 py-4 gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  size="sm"
                  variant={
                    chartFilters.hourly.interactions ? "solid" : "bordered"
                  }
            ...
            <Button
              ...
              onPress={() =>
                setFullscreenChart(
                  fullscreenChart === "hourly" ? null : "hourly",
                )
              }
            >
              {fullscreenChart === "hourly" ? (
          ...
          <CardBody
            className={
              fullscreenChart === "hourly" ? "h-[calc(100vh-100px)]" : ""
            }
          >
            {metrics.hourly_stats.length > 0 ? (
              <ResponsiveContainer
                height={fullscreenChart === "hourly" ? "100%" : 300}
                width="100%"
              >
              ...
              <CardBody>
                <div className="space-y-2 max-h-72 overflow-y-auto pr-2">
                  {metrics.top_recommended.map((item, idx) => {
                    ...
                    const clickData = metrics.clicked_links?.find(
                      (c) => c.url === item.url,
                    );
      ...
      {/* Recent Items Grid */}
      <div
        className={`grid grid-cols-1 ${paginatedErrors && paginatedErrors.length > 0 ? "lg:grid-cols-2" : ""} gap-6`}
      >
        {/* Recent Errors */}
        {paginatedErrors && paginatedErrors.length > 0 && (
          <Card>
            ...
            <CardBody>
              <div
                className={`space-y-4 ${isLoadingErrors ? "opacity-50" : ""} max-h-[600px] overflow-y-auto pr-2`}
              >
        ...
        <Card>
          ...
          <CardBody>
            {paginatedInteractions.length > 0 ? (
              <>
                <div
                  className={`space-y-4 ${isLoadingInteractions ? "opacity-50" : ""} max-h-[600px] overflow-y-auto pr-2`}
                >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor:
                              widgetSettings?.background_color || "#f4f4f5",
                          }}
                        >
                          {metric.output !== "-" ? (
