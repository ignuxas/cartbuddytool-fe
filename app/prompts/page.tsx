...
export default function MasterPromptsPage() {
  ...
  const { promptText: systemDefaults, isLoading: systemLoading } =
    useSystemMasterPrompt(authKey || "");
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure? This prompt might be in use by widgets."))
      return;
    try {
      await authenticatedFetcher(
        `${config.serverUrl}/api/master-prompts/${id}/`,
        authKey!,
        {
          method: "DELETE",
        },
        ...
        <Accordion variant="bordered">
          ...
          >
            <div className="prose prose-sm max-w-none p-2">
              <p>
                Master Prompts define the core behavior, tool usage rules, and
                output formats for the AI agent. They are separate from
                &ldquo;Project Custom Instructions&rdquo; which are specific to
                the domain.
              </p>
              ...
              <ul className="list-disc pl-5">
                <li>
                  <strong>TOOL USE:</strong> Explain how{" "}
                  <code>Semantic Search</code> and <code>URL Fetcher</code>{" "}
                  should be used.
                </li>
                <li>
                  <strong>OUTPUT FORMAT:</strong> Define strict JSON or card
                  formats like <code>[PRODUCT_CARD]</code> or{" "}
                  <code>[LINK_CARD]</code>. The frontend relies on these tags to
                  render UI elements.
                </li>
                <li>
                  <strong>GUIDELINES:</strong> General rules for tone, accuracy,
                  and URL validity.
                </li>
              </ul>

              <h4 className="font-bold mt-4">
                System Base Prompt (Reference Only):
              </h4>
              <p className="text-xs text-default-500 mb-2">
                This is the original hardcoded prompt used if no DB prompt is
                active.
              </p>
            ...
            <>
              ...
              <ModalBody>
                ...
                <Textarea
                  ...
                  onValueChange={(v) =>
                    setFormData({ ...formData, prompt_text: v })
                  }
                />
              </ModalBody>
              <ModalFooter>
