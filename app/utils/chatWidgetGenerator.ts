...
export function getChatWidgetScript(config: ChatWidgetConfig): string {
  ...
  const {
    ...
    baseUrl = "",
    botIcon,
    showAdvertisement: showAdvertisement = true,
    initialBotMessage = "",
  } = config;
...
(function() {
    ...
    const {
        ...
        input_placeholder: inputPlaceholder,
        footer_text: footerText,
        show_advertisement: showAdvertisement = true,
        view_product_text: viewProductText,
        visit_page_text: visitPageText,
        webhook_url: webhookUrl,
    ...
    function createChatWindow() {
        ...
        windowDiv.innerHTML = \`
            ...
            <div class="chat-footer">
                ...
                <p style="text-align:center;font-size:0.8em;color:#999;margin-top:10px">
                    \${showAdvertisement ? \`\${language === 'lt' ? 'Sukurta' : 'Powered by'} <a href="https://cartbuddy.ai" target="_blank" style="color:inherit;text-decoration:underline;font-weight:600;">Cartbuddy.ai</a>\` : footerText}
                </p>
            </div>
        \`;
        ...
        messageList = windowDiv.querySelector('#messageList');
            ...
            // Show initial greeting message as first bot message (always shown, not stored in messages)
            if (initialBotMessage) {
                const greetingElement = createMessageElement(initialBotMessage, 'bot');
                greetingElement.setAttribute('data-initial-greeting', 'true');
                messageList.appendChild(greetingElement);
            }
        ...
        if (messages.length === 0) {
            ...
            suggestionChips.className = 'suggestion-chips' + (initialBotMessage ? ' chips-right' : '');
            ...
            // Append to messageList so it flows with the flex gap
            messageList.appendChild(suggestionChips);
        }
